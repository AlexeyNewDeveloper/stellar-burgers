...
type IUserInitialStateTemp = Omit<
  IUserInitialState,
  "user" | "editableUser"
> & { user: any; editableUser: any };
...
export const userReducer = (
  ...
  switch (action.type) {
    ...
    case LOGOUT_USER: {
      // {
      //   [name in keyof IUserInitialState]: IUserInitialState[name];
      // }
      const initialUserState: IUserInitialStateTemp = { ...state };
      for (const key in initialUserState) {
        initialUserState[key as keyof IUserInitialState] = false;
      }
      return {
        ...initialUserState,
        user: null,
